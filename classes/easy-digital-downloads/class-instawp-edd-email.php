...
use EDD\Emails\Types;

/**
 * InstaWP_WaaS_EDD_Email
 */
if ( ! class_exists( 'InstaWP_WaaS_EDD_Email' ) ) {

    class InstaWP_WaaS_EDD_Email extends Types\Email {

        use Types\LegacyPaymentFilters;
        ...
        protected function set_email_body_content() {
            $option_value           = edd_get_option( 'instawp_notification', false );
            $this->raw_body_content = $option_value ? stripslashes( $option_value ) : $this->get_default_body_content();

            $this->maybe_run_legacy_filter( 'edd_instawp_notification_' . $this->processor()->get_template() );
            $this->maybe_run_legacy_filter( 'edd_instawp_notification' );

            $this->raw_body_content = apply_filters( 'edd_email_template_wpautop', true ) ? wpautop( $this->raw_body_content ) : $this->raw_body_content;
            $this->raw_body_content = apply_filters( 'edd_instawp_waas_' . $this->processor()->get_template(), $this->raw_body_content, $this->order );
            $this->raw_body_content = apply_filters( 'edd_instawp_waas', $this->raw_body_content, $this->order );
        }
        ...
        protected function set_from_name() {
            ...
            $this->from_name = apply_filters( 'edd_instawp_waas_from_name', $this->from_name, $this->order );
        }
        ...
        protected function set_from_email() {
            ...
            $this->from_email = apply_filters( 'edd_instawp_waas_from_email', $this->from_email, $this->order );
        }
        ...
        protected function set_headers() {
            ...
            $this->maybe_run_legacy_filter( 'edd_instawp_waas_headers' );

            $this->headers = apply_filters( 'edd_instawp_waas_headers', $this->headers, $this->order );
        }
        ...
        protected function set_subject() {
            $this->subject = edd_get_option( 'instawp_notification_subject', __( 'InstaWP WaaS', 'iwp-waas-integration' ) );

            $this->maybe_run_legacy_filter( 'edd_purchase_subject' );

            $this->subject = apply_filters( 'edd_instawp_waas_email_subject', $this->subject, $this->order );
        ...
        protected function set_heading() {
            $this->heading = edd_get_option( 'instawp_notification_heading', __( 'InstaWP WaaS', 'iwp-waas-integration' ) );

            $this->maybe_run_legacy_filter( 'edd_purchase_heading' );

            $this->heading = apply_filters( 'edd_instawp_waas_email_heading', wp_strip_all_tags( $this->heading ), $this->order );
        ...
        protected function set_attachments() {
            ...
            $this->maybe_run_legacy_filter( 'edd_instawp_waas_attachments' );

            $this->attachments = apply_filters( 'edd_instawp_waas_email_attachments', $this->attachments, $this->order );
        }
        ...
        public function get_default_body_content() {
            $default_email_body  = __( 'Dear', 'iwp-waas-integration' ) . " {name},\n\n";
            $default_email_body .= __( 'Here are the details of InstaWP WaaS:', 'iwp-waas-integration' ) . "\n\n";
            $default_email_body .= '{waas_list}' . "\n\n";
        ...
        protected function set_legacy_filters() {
            ...
            $this->legacy_filters = array(
                ...
                'edd_instawp_waas_attachments'                 => array(
                    'has_filter' => has_filter( 'edd_instawp_waas_attachments' ),
                    'property'   => 'attachments',
                    'arguments'  => array( 'order_id', 'payment_meta' ),
                ),
                'edd_instawp_waas_headers'                     => array(
                    'has_filter' => has_filter( 'edd_instawp_waas_headers' ),
                    'property'   => 'headers',
                    'arguments'  => array( 'order_id', 'payment_meta' ),
                ),
                'edd_instawp_notification_' . $email_template => array(
                    'has_filter' => has_filter( 'edd_instawp_notification_' . $email_template ),
                    'property'   => 'raw_body_content',
                    'arguments'  => array( 'order_id', 'payment_meta' ),
                ),
                'edd_instawp_notification'                    => array(
                    'has_filter' => has_filter( 'edd_instawp_notification' ),
                    'property'   => 'raw_body_content',
                    'arguments'  => array( 'order_id', 'payment_meta' ),
                ),
            );
